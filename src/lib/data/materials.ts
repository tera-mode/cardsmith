...
export const MATERIALS: Material[] = [
  ...
  // ─── スキル ──────────────────────────────────────────────────────────────────
  // 既存5種
  { id: 'skill_pen_3', name: '貫通', category: 'skill', description: '3回まで、前方の敵を貫通して攻撃', cost: 5, icon: '✨',
    effect: { type: 'skill', skill: { id: 'penetrate', uses: 3 } } },
  { id: 'skill_big_pen_1', name: '大貫通', category: 'skill', description: '1回だけ、盤面全貫通で攻撃', cost: 5, icon: '⚡',
    effect: { type: 'skill', skill: { id: 'big_penetrate', uses: 1 } } },
  { id: 'skill_counter_inf', name: '反撃', category: 'skill', description: '攻撃を受けるたびに反撃する', cost: 9, icon: '🔄',
    effect: { type: 'skill', skill: { id: 'hangeki', uses: 'infinite' } } },
  { id: 'skill_buff_1', name: '強化', category: 'skill', description: '1回だけ、ATKを+2する', cost: 4, icon: '💪',
    effect: { type: 'skill', skill: { id: 'buff', uses: 1 } } },
  { id: 'skill_heal_inf', name: '治癒', category: 'skill', description: '隣接する味方1体のHP+2（無限）', cost: 10, icon: '💚',
    effect: { type: 'skill', skill: { id: 'heal', uses: 'infinite' } } },

  // 攻撃時スキル
  { id: 'skill_rengeki_inf', name: '連撃', category: 'skill', description: '攻撃命中時、同じ対象に再度ATKダメージ', cost: 8, icon: '⚔️',
    effect: { type: 'skill', skill: { id: 'rengeki', uses: 'infinite' } } },
  { id: 'skill_nagibarai_inf', name: '薙ぎ払い', category: 'skill', description: '攻撃時、左右の敵にもATKダメージを与える', cost: 7, icon: '🌊',
    effect: { type: 'skill', skill: { id: 'nagibarai', uses: 'infinite' } } },
  { id: 'skill_touketsu_inf', name: '凍結', category: 'skill', description: '攻撃命中時、対象を次ターン行動不能にする', cost: 7, icon: '❄️',
    effect: { type: 'skill', skill: { id: 'touketsu', uses: 'infinite' } } },
  { id: 'skill_chinmoku_inf', name: '沈黙', category: 'skill', description: '攻撃命中時、対象のスキルを封印する', cost: 6, icon: '🤫',
    effect: { type: 'skill', skill: { id: 'chinmoku', uses: 'infinite' } } },
  { id: 'skill_fukitobashi_inf', name: '吹き飛ばし', category: 'skill', description: '攻撃命中時、対象を1マス吹き飛ばす（盤外で即死）', cost: 6, icon: '💨',
    effect: { type: 'skill', skill: { id: 'fukitobashi', uses: 'infinite' } } },
  { id: 'skill_kyuuketsu_inf', name: '吸血', category: 'skill', description: '攻撃命中時、与えたダメージ分HP回復', cost: 8, icon: '🩸',
    effect: { type: 'skill', skill: { id: 'kyuuketsu', uses: 'infinite' } } },
  { id: 'skill_rensa_3', name: '連鎖雷撃', category: 'skill', description: '攻撃時、対象の隣接敵1体にATK/2の連鎖ダメージ（3回）', cost: 6, icon: '⛓️',
    effect: { type: 'skill', skill: { id: 'rensa_raigeki', uses: 3 } } },

  // 被ダメージ時スキル
  { id: 'skill_hansha_inf', name: '反射', category: 'skill', description: 'ダメージを受けたとき、同量を攻撃者に返す', cost: 7, icon: '🪞',
    effect: { type: 'skill', skill: { id: 'hansha', uses: 'infinite' } } },
  { id: 'skill_ikari_inf', name: '怒り', category: 'skill', description: 'ダメージを受けるたびに自分のATK+1', cost: 6, icon: '😡',
    effect: { type: 'skill', skill: { id: 'ikari', uses: 'infinite' } } },

  // 死亡時スキル
  { id: 'skill_junkyou_1', name: '殉教', category: 'skill', description: '死亡時、味方全員のHPを2回復（1回）', cost: 6, icon: '🕊️',
    effect: { type: 'skill', skill: { id: 'junkyou', uses: 1 } } },
  { id: 'skill_shinigiwa_1', name: '死に際の咆哮', category: 'skill', description: '死亡時、隣接する味方のATK+2（1回）', cost: 6, icon: '💀',
    effect: { type: 'skill', skill: { id: 'shinigiwa', uses: 1 } } },
  { id: 'skill_fukkatsu_1', name: '復活', category: 'skill', description: '死亡した次のターン、同じ場所に復活（1回）', cost: 9, icon: '🔮',
    effect: { type: 'skill', skill: { id: 'fukkatsu', uses: 1 } } },
  { id: 'skill_zangai_1', name: '残骸', category: 'skill', description: '死亡時、その場所に1/1トークンを残す（1回）', cost: 4, icon: '🪨',
    effect: { type: 'skill', skill: { id: 'zangai', uses: 1 } } },

  // ターン終了時スキル
  { id: 'skill_saisei_inf', name: '再生', category: 'skill', description: 'ターン終了時、自分のHPを1回復', cost: 6, icon: '♻️',
    effect: { type: 'skill', skill: { id: 'saisei', uses: 'infinite' } } },

  // ターン開始時スキル
  { id: 'skill_haru_inf', name: '春の息吹', category: 'skill', description: 'ターン開始時、味方全員のHP+1回復', cost: 10, icon: '🌸',
    effect: { type: 'skill', skill: { id: 'haru_no_ibuki', uses: 'infinite' } } },
  { id: 'skill_shikikan_inf', name: '指揮官', category: 'skill', description: 'ターン開始時、カードを1枚追加ドロー', cost: 8, icon: '👑',
    effect: { type: 'skill', skill: { id: 'shikikan', uses: 'infinite' } } },
  { id: 'skill_shi_no_ryouiki_inf', name: '死の領域', category: 'skill', description: '敵ターン開始時、全敵に1ダメージ', cost: 9, icon: '☠️',
    effect: { type: 'skill', skill: { id: 'shi_no_ryouiki', uses: 'infinite' } } },

  // 召喚時スキル
  { id: 'skill_keigan_1', name: '慧眼', category: 'skill', description: '召喚時、カードを1枚ドロー（1回）', cost: 4, icon: '👁️',
    effect: { type: 'skill', skill: { id: 'keigan', uses: 1 } } },
  { id: 'skill_gaisen_1', name: '凱旋', category: 'skill', description: '召喚時、味方全員のHP+1（1回）', cost: 5, icon: '🏆',
    effect: { type: 'skill', skill: { id: 'gaisen', uses: 1 } } },
  { id: 'skill_gouka_1', name: '業火', category: 'skill', description: '召喚時、周囲8方向の敵にATK/2ダメージ（1回）', cost: 5, icon: '🔥',
    effect: { type: 'skill', skill: { id: 'gouka', uses: 1 } } },
  { id: 'skill_shoushuu_1', name: '召集', category: 'skill', description: '召喚時、隣接する空きマスに1/1トークン最大2体（1回）', cost: 5, icon: '🐣',
    effect: { type: 'skill', skill: { id: 'shoushuu', uses: 1 } } },

  // オーラスキル
  { id: 'skill_senki_inf', name: '戦旗', category: 'skill', description: '味方全員（自分以外）ATK+1オーラ', cost: 9, icon: '🚩',
    effect: { type: 'skill', skill: { id: 'senki', uses: 'infinite' } } },
  { id: 'skill_keigen_inf', name: '軽減', category: 'skill', description: '受けるダメージを1軽減（最小1）', cost: 7, icon: '🛡️',
    effect: { type: 'skill', skill: { id: 'keigen', uses: 'infinite' } } },
  { id: 'skill_koutetsu_1', name: '鋼鉄の意志', category: 'skill', description: '致死ダメージを1度だけHP1で耐える（1回）', cost: 7, icon: '🦾',
    effect: { type: 'skill', skill: { id: 'koutetsu_no_ishi', uses: 1 } } },
  { id: 'skill_kyousenshi_inf', name: '狂戦士化', category: 'skill', description: 'HP50%以下のとき自分ATK+2オーラ', cost: 7, icon: '🩹',
    effect: { type: 'skill', skill: { id: 'kyousenshi', uses: 'infinite' } } },
  { id: 'skill_seinaru_inf', name: '聖なる加護', category: 'skill', description: '味方全員の最大HP+1オーラ', cost: 9, icon: '⭐',
    effect: { type: 'skill', skill: { id: 'seinaru_kago', uses: 'infinite' } } },

  // 起動型スキル
  { id: 'skill_mahi_3', name: '麻痺', category: 'skill', description: '敵1体を次ターン行動不能に（3回）', cost: 5, icon: '💫',
    effect: { type: 'skill', skill: { id: 'mahi', uses: 3 } } },
  { id: 'skill_hikiyose_inf', name: '引き寄せ', category: 'skill', description: '3射程内の敵を隣接マスに引き寄せる', cost: 5, icon: '🧲',
    effect: { type: 'skill', skill: { id: 'hikiyose', uses: 'infinite' } } },
  { id: 'skill_irekae_1', name: '位置入れ替え', category: 'skill', description: '味方ユニットと位置を交換（1回）', cost: 4, icon: '🔀',
    effect: { type: 'skill', skill: { id: 'irekae', uses: 1 } } },
  { id: 'skill_shunkan_idou_1', name: '瞬間移動', category: 'skill', description: '任意の空きマスへ瞬間移動（1回）', cost: 5, icon: '🌀',
    effect: { type: 'skill', skill: { id: 'shunkan_idou', uses: 1 } } },
  { id: 'skill_jibaku_1', name: '自爆', category: 'skill', description: '自身を犠牲に周囲8方向にATK×2ダメージ（1回）', cost: 5, icon: '💥',
    effect: { type: 'skill', skill: { id: 'jibaku', uses: 1 } } },
  { id: 'skill_kagebunshin_3', name: '影分身', category: 'skill', description: '隣接する空きマスに能力なしの分身を生成（3回）', cost: 7, icon: '👥',
    effect: { type: 'skill', skill: { id: 'kagebunshin', uses: 1 } } },
  { id: 'skill_shoukanshi_1', name: '召喚士', category: 'skill', description: 'コスト6以下のカードを手札から即召喚（1回）', cost: 5, icon: '📜',
    effect: { type: 'skill', skill: { id: 'shoukanshi', uses: 1 } } },
  { id: 'skill_tenkei_1', name: '天啓', category: 'skill', description: '手札1枚のコストを3減らす（1回）', cost: 4, icon: '💡',
    effect: { type: 'skill', skill: { id: 'tenkei', uses: 1 } } },
  { id: 'skill_daishinkan_1', name: '大震撼', category: 'skill', description: '全敵に1ダメージ（1回）', cost: 5, icon: '⚡',
    effect: { type: 'skill', skill: { id: 'daishinkan', uses: 1 } } },

  // ── 攻撃時スキル (v2) ──────────────────────────────────────────────────
  { id: 'skill_shinshoku_inf', name: '侵食', category: 'skill', description: '攻撃で敵を倒すたびにATK+1', cost: 7, icon: '🌑',
    effect: { type: 'skill', skill: { id: 'shinshoku', uses: 'infinite' } } },
  { id: 'skill_chodan_inf', name: '跳弾', category: 'skill', description: '攻撃時、ATK/3のダメージをランダムな別の敵に与える', cost: 6, icon: '🎯',
    effect: { type: 'skill', skill: { id: 'chodan', uses: 'infinite' } } },
  { id: 'skill_mujihi_inf', name: '無慈悲', category: 'skill', description: 'HP50%以下の敵を攻撃したとき即死させる', cost: 8, icon: '💀',
    effect: { type: 'skill', skill: { id: 'mujihi', uses: 'infinite' } } },
  { id: 'skill_buki_hakai_inf', name: '武器破壊', category: 'skill', description: '攻撃命中時、対象のATK-1（永続）', cost: 6, icon: '🔨',
    effect: { type: 'skill', skill: { id: 'buki_hakai', uses: 'infinite' } } },

  // ── 被ダメージ時スキル (v2) ────────────────────────────────────────────
  { id: 'skill_teni_3', name: '転移', category: 'skill', description: 'ダメージを受けたとき、ランダムな空きマスに移動（3回）', cost: 5, icon: '🌀',
    effect: { type: 'skill', skill: { id: 'teni', uses: 3 } } },
  { id: 'skill_touketsu_hansha_inf', name: '凍結反射', category: 'skill', description: 'ダメージを受けたとき、攻撃者を凍結させる', cost: 7, icon: '🧊',
    effect: { type: 'skill', skill: { id: 'touketsu_hansha', uses: 'infinite' } } },
  { id: 'skill_bankai_1', name: '万解', category: 'skill', description: '初めてHP50%以下になった時にHP+2（1回）', cost: 5, icon: '🛡️',
    effect: { type: 'skill', skill: { id: 'bankai', uses: 1 } } },

  // ── 死亡時スキル (v2) ──────────────────────────────────────────────────
  { id: 'skill_eiyou_1', name: '栄養', category: 'skill', description: '死亡時、味方全員のATK+1（1回）', cost: 7, icon: '🦅',
    effect: { type: 'skill', skill: { id: 'eiyou', uses: 1 } } },
  { id: 'skill_daishou_1', name: '代償の魂', category: 'skill', description: '死亡時、自陣HP+2（1回）', cost: 5, icon: '💎',
    effect: { type: 'skill', skill: { id: 'daishou_no_tamashii', uses: 1 } } },
  { id: 'skill_saikouchiku_1', name: '再構築', category: 'skill', description: '死亡時、手札の最低コストカードを即召喚（1回）', cost: 7, icon: '🔁',
    effect: { type: 'skill', skill: { id: 'saikouchiku', uses: 1 } } },
  { id: 'skill_hatsuga_1', name: '発芽', category: 'skill', description: '死亡時、そのマスに1/1の苗トークンを生成（1回）', cost: 4, icon: '🌱',
    effect: { type: 'skill', skill: { id: 'hatsuga', uses: 1 } } },

  // ── ターン終了/開始スキル (v2) ─────────────────────────────────────────
  { id: 'skill_dokumu_inf', name: '毒霧', category: 'skill', description: '自ターン終了時、隣接全敵に1ダメージ', cost: 7, icon: '☁️',
    effect: { type: 'skill', skill: { id: 'dokumu', uses: 'infinite' } } },
  { id: 'skill_kunren_inf', name: '訓練', category: 'skill', description: '自ターン開始時、隣接する味方1体のATK+1', cost: 7, icon: '⚒️',
    effect: { type: 'skill', skill: { id: 'kunren', uses: 'infinite' } } },

  // ── オーラスキル (v2) ──────────────────────────────────────────────────
  { id: 'skill_shireijutsu_inf', name: '死霊術', category: 'skill', description: '味方が死ぬたびにATK+1（蓄積）', cost: 8, icon: '💀',
    effect: { type: 'skill', skill: { id: 'shireijutsu', uses: 'infinite' } } },
  { id: 'skill_mure_inf', name: '群れ', category: 'skill', description: '同系統の味方が3体以上の時、ATK+2', cost: 6, icon: '🐺',
    effect: { type: 'skill', skill: { id: 'mure', uses: 'infinite' } } },
  { id: 'skill_kokou_inf', name: '孤高', category: 'skill', description: '周囲8方向に味方がいない時、ATK+3', cost: 7, icon: '🦁',
    effect: { type: 'skill', skill: { id: 'kokou', uses: 'infinite' } } },

  // ── 起動型スキル (v2) ──────────────────────────────────────────────────
  { id: 'skill_ansatsu_1', name: '暗殺', category: 'skill', description: '背後にいる敵を即死させる（1回）', cost: 8, icon: '🗡️',
    effect: { type: 'skill', skill: { id: 'ansatsu', uses: 1 } } },
  { id: 'skill_kakusei_1', name: '覚醒', category: 'skill', description: '味方1体のATK+2、HP+2（1回）', cost: 6, icon: '✨',
    effect: { type: 'skill', skill: { id: 'kakusei', uses: 1 } } },
  { id: 'skill_abekobe_1', name: 'あべこべ', category: 'skill', description: '敵1体のATKと現在HPを入れ替える（1回）', cost: 5, icon: '🔄',
    effect: { type: 'skill', skill: { id: 'abekobe', uses: 1 } } },
  { id: 'skill_soukan_1', name: '送還', category: 'skill', description: '隣接する敵を手札に戻す（1回）', cost: 6, icon: '↩️',
    effect: { type: 'skill', skill: { id: 'soukan', uses: 1 } } },

  // ── on_move スキル ─────────────────────────────────────────────────────
  { id: 'skill_shissou_3', name: '疾走', category: 'skill', description: '移動するたびにATK+1（最大3回）', cost: 5, icon: '💨',
    effect: { type: 'skill', skill: { id: 'shissou', uses: 3 } } },
  { id: 'skill_zankyou_inf', name: '残響', category: 'skill', description: '移動するたびに元のマスに1/1トークン', cost: 6, icon: '🌊',
    effect: { type: 'skill', skill: { id: 'zankyou', uses: 'infinite' } } },
  { id: 'skill_junrei_inf', name: '巡礼', category: 'skill', description: '前衛列に到達するたびに自陣HP+1', cost: 5, icon: '⛩️',
    effect: { type: 'skill', skill: { id: 'junrei', uses: 'infinite' } } },
  { id: 'skill_kazeyomi_inf', name: '風読み', category: 'skill', description: '移動するたびに次の攻撃ATK+1（蓄積）', cost: 5, icon: '🌬️',
    effect: { type: 'skill', skill: { id: 'kazeyomi', uses: 'infinite' } } },
  { id: 'skill_tsuiseki_inf', name: '追跡', category: 'skill', description: '移動後、最近接敵との距離分だけ次攻撃ATKボーナス', cost: 5, icon: '🎯',
    effect: { type: 'skill', skill: { id: 'tsuiseki', uses: 'infinite' } } },

  // ── on_skill_used スキル ───────────────────────────────────────────────
  { id: 'skill_maryoku_inf', name: '魔力増幅', category: 'skill', description: 'スキル使用を観測するたびにATK+1', cost: 7, icon: '🔮',
    effect: { type: 'skill', skill: { id: 'maryoku_zoufuku', uses: 'infinite' } } },
  { id: 'skill_rendou_5', name: '連動', category: 'skill', description: '味方スキル使用時、そのユニットのスキル回数+1（5回）', cost: 6, icon: '⛓️',
    effect: { type: 'skill', skill: { id: 'rendou', uses: 5 } } },
  { id: 'skill_shokubai_inf', name: '触媒', category: 'skill', description: '味方スキル使用時に自陣HP+1', cost: 7, icon: '🧪',
    effect: { type: 'skill', skill: { id: 'shokubai', uses: 'infinite' } } },
  { id: 'skill_kyuushuu_inf', name: '吸収', category: 'skill', description: '敵スキル使用時にHP+1', cost: 5, icon: '🔋',
    effect: { type: 'skill', skill: { id: 'kyuushuu', uses: 'infinite' } } },
  { id: 'skill_rensajumon_inf', name: '連鎖呪文', category: 'skill', description: '味方スキル使用時にランダムな敵に1ダメージ', cost: 7, icon: '⚡',
    effect: { type: 'skill', skill: { id: 'rensajumon', uses: 'infinite' } } },

  // ── on_summon_ally スキル ──────────────────────────────────────────────
  { id: 'skill_shukufuku_inf', name: '祝福', category: 'skill', description: '味方召喚時、その味方のHP+1', cost: 6, icon: '🌟',
    effect: { type: 'skill', skill: { id: 'shukufuku', uses: 'infinite' } } },
  { id: 'skill_kobu_summon_inf', name: '鼓舞召喚', category: 'skill', description: '味方召喚時、その味方のATK+1', cost: 6, icon: '📯',
    effect: { type: 'skill', skill: { id: 'kobu_summon', uses: 'infinite' } } },
  { id: 'skill_kangei_inf', name: '歓迎の儀', category: 'skill', description: '味方召喚時に自陣HP+1', cost: 5, icon: '🎊',
    effect: { type: 'skill', skill: { id: 'kangei_no_gi', uses: 'infinite' } } },
  { id: 'skill_kyoumei_inf', name: '共鳴', category: 'skill', description: '味方召喚時に自分のATK+1', cost: 6, icon: '🎵',
    effect: { type: 'skill', skill: { id: 'kyoumei', uses: 'infinite' } } },
  { id: 'skill_summon_chain_3', name: '召喚連鎖', category: 'skill', description: '味方召喚時、その味方が即座に行動可能（3回）', cost: 7, icon: '🔗',
    effect: { type: 'skill', skill: { id: 'summon_chain', uses: 3 } } },

  // ── on_base_damaged スキル ─────────────────────────────────────────────
  { id: 'skill_chuusei_inf', name: '忠誠', category: 'skill', description: '自陣がダメージを受けるたびにATK+2', cost: 7, icon: '🛡️',
    effect: { type: 'skill', skill: { id: 'chuusei', uses: 'infinite' } } },
  { id: 'skill_houfuku_inf', name: '報復の刃', category: 'skill', description: '自陣ダメージ時に最近接の敵に1ダメージ', cost: 6, icon: '⚔️',
    effect: { type: 'skill', skill: { id: 'houfuku_no_yaiba', uses: 'infinite' } } },
  { id: 'skill_douin_inf', name: '動員', category: 'skill', description: '自陣ダメージ時にカード1枚ドロー', cost: 7, icon: '📜',
    effect: { type: 'skill', skill: { id: 'douin', uses: 'infinite' } } },
  { id: 'skill_dohatsu_inf', name: '怒髪', category: 'skill', description: '自陣ダメージ時にHP+1・ATK+1', cost: 7, icon: '😤',
    effect: { type: 'skill', skill: { id: 'dohatsu', uses: 'infinite' } } },
  { id: 'skill_gekibun_inf', name: '檄文', category: 'skill', description: '自陣ダメージ時に味方全員のATK+1', cost: 9, icon: '📣',
...
export function formatSkillUses(uses: number | 'infinite'): string {
  return uses === 'infinite' ? '∞' : `×${uses}`;
}
